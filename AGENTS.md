...
| PowerDNS | 5.1.3 | Both nodes Native (read-write), shared PostgreSQL HA backend |
| Traefik | 3.7 | Reverse proxy, DNS-01 ACME via PowerDNS API |
| PostgreSQL | 18 | Streaming replication via repmgr 5.5, auto-failover |
| NetBird | 0.74.7 | Combined management+signal+relay, PostgreSQL backend, active-passive DNS, primary-only (fenced, gotchas 149/150) |
| NetBird Reverse Proxy | 0.74.7 | Public HTTPS ingress to NetBird-mesh services. Sits behind Traefik in TCP-passthrough mode at `*.proxy.<domain>`; terminates TLS itself with ACME TLS-ALPN-01. Version must track the server (0.73.x showed skew breaks it). |
| Zitadel | 4.16.1 | Central IAM (OIDC/OAuth2), PostgreSQL backend, multi-tenant |
| Gatus | 5.36.0 | HA monitoring dashboard + alert receiver, PostgreSQL backend |
| OpenZiti | 1.6.17 | Zero-trust network overlay, active-standby controller, active-active edge routers |
| Portainer | 2.39.5 | Docker management UI, WireGuard + NetBird access |
| Restic | 0.19.1 | Incremental backup to SFTP backup server |
...
| 148 | **External peers strand after every netbird-server restart** (gotcha #136 follow-on). NetBird signal-state is in-memory in netbird-server. Any restart wipes it — and netbird-server MUST restart on PG failover because both its connection pools (pgx + lib-pq via GORM) cache stale connections to the ex-primary for up to 60 minutes (`MaxConnLifetime`) with no `PrepareConn`/`BeforeAcquire` validation hooks (verified in v0.69.0 source `management/server/store/sql_store.go:62-147,2794-2835`). Empirically observed during 2026-04-25 destructive failover test: `ValidateConnect failed (read only connection)` in netbird-server logs, three external customer peers stranded for 3+ hours. **Affected events**: destructive failover test, server reboot, hardware outage, image upgrade, any container restart. **Local peers (ns1/ns2) recover** via the watchdog's `systemctl restart netbird`. **External peers (anything not ns1/ns2)** retain stale signal sessions and need manual `sudo netbird down && sudo netbird up` on each affected machine — there is no remote way to kick them. The watchdog now invokes `/usr/local/bin/netbird-peer-wakeup.sh` after every role change: pings each peer's NetBird IP from ns1/ns2 (forces an ICE offer from our side via signal — recovers ~50% of cases), then alerts via Gatus `NetBird-Mesh-Peers-Healthy` for any still-stale peers. **Operator runbook**: after any planned netbird-server restart, monitor the `NetBird-Mesh-Peers-Healthy` Gatus endpoint and notify owners of any flagged peers to bounce their daemon. **Update 2026-07 — three structural fixes shrink this class**: (1) netbird-server no longer restarts on PG failover — the watchdog fences it to the primary (gotcha 149) and it starts FRESH on the new primary after pgproxy retarget, so its pools cannot poison and signal state is only wiped when all clients must reconnect anyway; (2) pgproxy kills orphaned per-connection socat children on target change (with `fork`, children survive the parent kill and kept proxying pooled consumer connections to the read-only ex-primary — the original poisoning path); (3) peers run a client-side self-heal guard (`netbird-selfheal.timer`, deployed by the netbird_peer role; for unmanaged customer machines `scripts/` has per-platform variants — `netbird-selfheal-install.sh` for systemd Linux, `netbird-selfheal-install.ps1` for Windows Scheduled Task, `netbird-selfheal-docker.sh` for cron + containerized clients, `netbird-selfheal-compose.example.yml` for compose-only healthcheck+autoheal) that performs the `netbird down && up` cycle (or `docker restart`) automatically after ~6 min of management/signal disconnect — the only mechanism that recovers external peers after a hard primary failure. Wake-up ping + Gatus alert remain for unguarded peers; `netbird-peer-health.sh` now prefers the management API (`netbird_admin_pat`) for authoritative stranded-peer lists (`connected` + `last_seen` per peer). **Correction 2026-07-22**: the wedge is NOT version-bound — during the 2026-07-22 drill, 6 external peers including a **current 0.74.7 client** wedged permanently after a ~90s Traefik stop (clean connection-refused; signal/relay/p2p kept half-living while management never re-registered; DB showed stale `connected=true`). A client version floor is therefore hygiene, not a stranding fix — **the self-heal guard is the only effective protection for external peers**. Beware: `peer_status_connected` in the DB is not trustworthy after server restarts; use `peer_status_last_seen` to spot stranded peers. |
| 149 | **netbird-server + netbird-proxy are intentionally STOPPED on the PG standby** (fencing). NetBird signal state is per-instance and in-memory — a standby that accepts client connections re-opens the split-signal trap (gotcha 136) for any client with stale DNS. A stopped container also drops the standby's Traefik netbird routes, so a stale client gets an immediate failure → redials → 30s-TTL DNS → primary. | Enforced by `enforce_netbird_fence()` in `pg-role-watchdog.sh` every cycle: stop on standby, start on primary AFTER pgproxy retarget (starting earlier would poison the activity-store pool via the stale pgproxy target). `docker start netbird-server` on the standby gets reverted within ~30s — self-healing, not a malfunction. Because of `restart: unless-stopped`, a fenced container stays stopped across reboots: ONLY the watchdog starts netbird-server on promotion. Deploy roles (netbird_management, netbird_proxy) and rolling-update.yml fence inline post-`compose up` to close the 30s window. Do not "fix" a stopped netbird-server/netbird-proxy without checking `pg_is_in_recovery()` first. Test suite asserts running-on-primary/stopped-on-standby. **Accepted trade-off**: on every failover there is a short window (bounded by the two nodes' uncoordinated 30s watchdog timers + the ~7s pgproxy retarget wait) with no netbird-server running anywhere — clients just retry and land on the new primary; this replaces the far worse silent split-signal stranding. **Residual closed by the fence sentinel (gotcha 150)**: the fenced standby also stops netbird-dashboard and runs the `netbird-fence` sentinel, whose TCP-passthrough-to-dead-port router closes vpn-domain connections mid-TLS — stale-DNS clients get a retryable transport error instead of the channel-pinning 404. |
| 150 | **Management-transport pinning (fully characterized 2026-07-22)** — the actual root cause of the "dangling client" phenomenon, version-INDEPENDENT (reproduced on client 0.74.7). If the management gRPC channel connects to a live-but-wrong HTTP endpoint (LB with valid TLS whose mgmt backend is absent → HTTP 404 → gRPC `Unimplemented`), the channel loops `state READY → RPC fails` forever: a 404 is a valid HTTP response over a healthy h2 transport, so no transport error ever occurs, the channel never re-resolves DNS and never redials. Controlled tests: clean RST outage → client retries ~60s and recovers ~30s after restore (NO bug); live-404 endpoint → **zero recovery in 30 min** after the fault cleared, while signal AND relay reconnected within seconds through the same hostname. Production: 6 external peers (0.66.2→0.74.7) pinned for 5.5h after a 90s Traefik stop + failover. | Three effective layers: (1) **the `netbird-fence` sentinel** (platform-side prevention, validated live): a busybox container run ONLY on the standby whose Traefik labels define a TCP router `HostSNI(vpn domain)` + `tls.passthrough` to a dead port — connections are closed mid-TLS-handshake (retryable transport error) instead of answered. CRITICAL detail: TLS termination beats TCP passthrough while ANY Host-matching HTTP router lives (priority does not help — observed), so the standby fence must also stop **netbird-dashboard**; ordering (sentinel up -> dashboard down -> proxy down -> server down last / reverse on promotion) keeps the SNI owned by a correct answerer or the sentinel at every instant. (2) the client-side self-heal guard (detects mgmt-disconnected >~6 min, forces `netbird down && up` = fresh channel + fresh DNS — proven instant remedy, also covers pre-existing pinned peers and unknown conditions); (3) the upstream fix (draft: `docs/upstream/netbird-management-transport-pinning.md`). NOT effective: client version floors (bug present in current client), plain container-stop fencing (Traefik/dashboard 404s and 502s all pin — any live HTTP answer keeps the transport healthy), wake-up pings (pinned peers have no signal session to nudge). Diagnosis marker: client log shows `management connection state READY` + `failed getting Management Service public key: ... Unimplemented ... 404` repeating; DB shows stale `connected=true` with frozen `peer_status_last_seen`. Repro harness: `/var/tmp/wedge-test4.sh` on ns2. **CRITICAL addendum (2026-07-22 failback)**: the sentinel only governs NEW dials — Traefik never terminates EXISTING h2 connections on router changes, and idleTimeout=0 keeps them alive forever, so already-connected clients stay pinned to the pre-transition router set regardless of fencing (observed: five peers, 6.7k requests/30min of Traefik-404s to the demoted node an hour after failback). The watchdog therefore kills all established :443 connections in Traefik's netns after every role transition (`nsenter -t <traefik pid> -n ss -K 'sport = :443'` — host ss cannot see container sockets, and unbracketed IPv6 `ss` filters silently match nothing). Server-side manual unpin for stuck peers: same command on the node they're pinned to. |
| 152 | **Self-heal guard was a silent no-op from day one** — its enrollment check tested `/etc/netbird/config.json`, but the client (profile-based layout, present since the Apr 2026 enrollment) stores config at `/var/lib/netbird/default.json`. The guard exited 0 on every run without logging, counters never incremented, and every "guard-equipped peers recover in ~6 min" assumption was untested fiction until the 2026-07-22 failback left both local clients pinned for 10+ minutes with the guard timer firing green. The netbird_peer role's enrollment `stat:` had the same single-path bug (would have re-enrolled enrolled peers if a setup key were passed). Root lesson: **a guard whose healthy path and whose early-exit path are both silent is unverifiable — and a behavioral test that fakes the environment (touched a fake config.json) validates the logic but not the deployment.** | Enrollment checks now match all known layouts (`/etc/netbird/config.json`, `/var/lib/netbird/default.json`, `/var/lib/netbird/config.json`) in the role template, the standalone installer, and the role's enrollment detection. Validated live: the fixed guard's first production firing recovered both failback-pinned clients. When touching guard-style scripts, verify on the real host that the check passes its gates (`bash -x`), not just that the timer is active. |
| 151 | **Live-standby WAL gap is not self-healing and was not alerted** — the entrypoint's WAL-gap auto-re-clone only fires when PG is stuck in STARTUP replay (never reaches pg_isready). If the gap develops while the standby is already up (walreceiver dies, primary recycles the segment — e.g. back-to-back node reboots outrunning `max_slot_wal_keep_size`, observed 2026-07-22), PG keeps serving reads and retries `requested WAL segment ... has already been removed` forever, silently falling behind. | Watchdog now alerts via Gatus (`PG-Standby-Streaming`) after ~2 min of a standby with no streaming WAL receiver. Remedy: `docker stop postgresql`, wipe the `postgresql_postgresql_data` volume contents, `docker start postgresql` — the entrypoint's fresh-standby path clones from the primary (537MB ≈ <1 min). Verify `pg_stat_wal_receiver` shows `streaming` after. |
| 153 | **OpenZiti conflated its PREFERRED controller home with the node currently RUNNING the controller.** `openziti_primary_node` defaulted to `{{ dns_failover_primary }}`, but `dns_failover_primary` is a *runtime* value — site.yml's pre-flight sets it to whoever holds the PG primary, and RECOVERY tells you to override it during a rebuild. So a PG failover silently re-designated OpenZiti's preferred primary, templating `ziti-watchdog` into STANDBY mode on both nodes (failback can then never fire) and installing the standby DB-sync timer onto the real primary (found live on ns1 2026-07-23, pointed at ns2, self-skipping only because ns1's controller was up). Worse, the role's `Detect active controller node` task was a STUB returning the preferred primary, so rebuilding the *preferred primary* while the controller lived on the peer would have (a) re-run the PKI bootstrap on the empty node, minting a BRAND-NEW root CA that invalidates every enrolled router and identity, and (b) started a second controller = split-brain. | Split into two variables: `openziti_primary_node` = static preferred home (drives watchdog mode, DB-sync direction, DNS records — now defaults to `groups['dns_servers'] \| first`, NOT dns_failover_primary), and `_openziti_active_controller` = detected each run by probing both nodes' edge API over WireGuard (preferred wins when healthy, so steady state never flaps). Bootstrap/PKI-source/container-start/handlers all gate on the ACTIVE controller; PKI now syncs *from* it, so a rebuilt primary pulls the surviving CA instead of minting one. A hard guard refuses to bootstrap when the local CA is absent and the peer cannot be proven CA-free (`-e openziti_force_bootstrap=true` to override — greenfield only). The role also removes the stale standby DB-sync units from the preferred primary. **Sub-gotcha:** a delegated task's connection `vars:` are templated in the DELEGATE's context, so a plain `set_fact` is undefined there under `--limit` (`'_openziti_active_controller' is undefined`) — the old code only worked because it used a role default; publish the fact to the peer with `delegate_to` + `delegate_facts: true`. |
| 154 | **`host_vars/` silently outranked the role default, so the 2026-07-22 backup-path fix was completely inert for a day.** The audit rewrote `roles/backup/defaults/main.yml` to back up all of `/opt`, but `host_vars/ns1.yml` and `host_vars/ns2.yml` each contained `restic_backup_paths: "{{ nsN_backup_paths }}"`, and host_vars beat role defaults. Every snapshot — including the "verified" post-audit ones — still used the stale list: no `/opt/openziti` (ziti controller DB **and** PKI, which are NOT in PostgreSQL), no `/opt/traefik`, no `/opt/netbird-proxy`, plus the long-removed `/opt/postgresql/backups`. Caught only by an actual restore: `restore.sh files --include /opt/openziti` returned "Restored 0 files". | Removed the `restic_backup_paths` line from both host_vars files (kept the files as tombstones explaining why). Root lesson: **a backup is only verified by restoring from it.** Checking the config, the exit code, or even `restic ls` of paths you expect to be there does not prove a path was ever *submitted* — read the path list echoed in restic's own `restoring snapshot ... of [<paths>]` output. Re-run `deploy-backup.yml` after any path change; the script on the node is what runs, not the repo. |
| 155 | **A PG18 `pg_dumpall` restored into a postgres MISSING an extension silently drops whole databases after the failure point — psql does not stop.** Found during the 2026-07-25 isolated pgdump load-test: loading the cluster dump into a stock `postgres:18` (no `repmgr` extension) failed `CREATE EXTENSION repmgr`, so the following `COPY repmgr.events FROM stdin` had no target table; psql then reads the ~1500 COPY-data lines as SQL, and an unbalanced quote inside a repmgr *event message* swallowed the subsequent `CREATE DATABASE zitadel` — so **zitadel (the auth DB) was silently absent** while every other DB loaded with exact fidelity. Two compounding PG18 specifics: (a) `pg_dumpall` now wraps each DB section in `\restrict <token>`/`\unrestrict <token>` (psql hardening — blocks all backslash meta-commands except `\unrestrict` while restricted); (b) the official `postgres:18` image moved `PGDATA` to `/var/lib/postgresql/18/docker`, so mount the volume at the PARENT `/var/lib/postgresql`, not `…/data` (else the container exits on start). | Always restore the cluster dump into the SAME image production uses (`sourcemation/postgres-repmgr:5.5.0`, which has repmgr) — `restore.sh database --load` targets the live `postgresql` container, so real recovery is unaffected. But `restore.sh --load` pipes `psql` with no `ON_ERROR_STOP` and no post-load check, so a silent partial restore (missing extension, wrong image) would report success. **After any restore, verify per-database table counts** — a fresh cluster + `query_to_xml` count-all script is in the 2026-07-25 test (baseline had e.g. zitadel=149 tables/3794 rows, pdnsadmin≈1.39M rows). Recommended hardening (not yet done): have `restore.sh --load` assert the expected database list exists afterward and surface any psql errors. To inspect a cluster dump WITHOUT repmgr, strip the repmgr DB and all `\restrict`/`\unrestrict` lines first (`awk` filter in the test notes). |
