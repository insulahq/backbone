...
Both ns1 and ns2 run the full NetBird stack behind Traefik (HTTPS). DNS round-robin on `netbird.<domain>` distributes clients between both nodes.

- **Database:** PostgreSQL HA (deployed by `postgresql_repmgr` role), multi-host DSN with `target_session_attrs=read-write`
- **IdP:** Dex embedded in NetBird management server. Credential store (`idp.db`) is SQLite, synced NS1->NS2 by Ansible on every deploy
...
After first deploy, complete the browser-based setup wizard at `https://netbird.<domain>`:
1. Create admin user
...
- `idp.db` is not replicated via PostgreSQL; Ansible syncs it NS1->NS2 (gotcha 39)
