...
| **Traffic Routing** | DNS-based ingress routing (NGINX DaemonSet + PowerDNS multi-A) | — | No hoster lock-in, no Floating IP, automatic DNS failover. See ADR-014. |
...
| **Admin Access VPN** | NetBird (WireGuard mesh) | Latest | Zero-trust admin access; SSH/kubectl via mesh only, not public internet. Self-hosted, OIDC via Dex. See ADR-013. |
