...
1. **Primary Mode** — Platform is authoritative; full DNS control
...
| **Redundant DNS** | ✅ Platform NS1/NS2/NS3 | ❌ None | ✅ Customer + Platform |
| **DDoS mitigation** | ✅ Via platform NS | ❌ None | ✅ Via customer NS |
...
Platform becomes the **authoritative nameserver** for the customer's domain. Customer delegates nameserver records at their registrar.

```
Customer Registrar (GoDaddy, Namecheap)
  ↓ (NS records point to)
Platform Nameservers:
  - ns1.k8s.local (Primary)
  - ns2.k8s.local (Secondary)
  - ns3.k8s.local (Tertiary, geo-redundant)
```
...
3. Admin sends customer instructions:
   ...
   Nameserver 1: ns1.k8s.local
   Nameserver 2: ns2.k8s.local
   Nameserver 3: ns3.k8s.local
   ```
...
2. Update nameserver records to:
   - `ns1.k8s.local`
   - `ns2.k8s.local`
   - `ns3.k8s.local`
...
- 🟢 **Redundancy** — 3 nameservers across regions (automatic failover)
...
- 🔴 **DDoS at platform DNS** — Query attacks target platform nameservers
- 🔴 **Single point of failure** — If platform DNS down, domain offline
...
|-------|---------|--------|
| Zone not delegated | NS records at registrar != platform NS after 48h | Email customer reminder |
| AXFR failed | ns2/ns3 can't sync zone 1+ hour | Page on-call, investigate DNS API |
...
Platform Secondary DNS (ns1.k8s.local)
...
2. Add secondary nameserver:
   - **Nameserver:** `ns1.k8s.local` (or platform-provided FQDN)
   - **IP:** `5.6.7.8` (platform's IP, provided by admin)
3. Some providers require SOA notification — admin sends this
4. Wait 24-48 hours for DNS propagation
5. Test with: `dig @ns1.k8s.local acme.com SOA`
...
```json
{
  ...
  "data": {
    ...
    "dns": {
      ...
      "instruction": "Add ns1.k8s.local (5.6.7.8) as secondary nameserver at your registrar",
      "axfr_status": "pending"
    }
  }
}
...
2. Every 3600 seconds (1 hour):
   - PowerDNS sends AXFR request to customer's primary NS
   - Zone data pulled: A, AAAA, MX, TXT, SOA records
   - Stored in platform's PowerDNS database
   - ns2/ns3 replicate via internal AXFR
   
3. DNS queries to platform return zone data:
   - Query: dig @ns1.k8s.local acme.com A
...
- ✅ **Stores zone data** in PowerDNS secondary database
- ✅ **Responds to queries** for backup redundancy
- ✅ **Replicates to ns2/ns3** for internal HA
...
3. Customer's Registrar Update:
   - Add secondary: ns1.k8s.local (5.6.7.8)
   - Wait for sync

4. Ongoing:
   - Customer updates records in Route53
   - Platform syncs via AXFR every 3600s
   - Both Route53 + platform respond to queries
   - If Route53 down, platform provides DNS fallback
```
...
| Alert | Trigger | Action |
|-------|---------|--------|
| AXFR failed | Zone sync from primary fails >1 hour | Page on-call, check primary NS health |
...
| **Redundancy** | ✅ 3 platform NS | ❌ Customer's only | ✅ Customer's + platform |
| **DDoS Protection** | At platform DNS | At customer's DNS | Both nameservers |
...
1. Keep primary zone active in PowerDNS
...
5. Admin removes PowerDNS zone delegation request
6. Keep PowerDNS zone for internal records only (MX still resolves via primary)

**Gotcha:** Email won't work if customer doesn't set up MX in their DNS

### Primary → Secondary

1. Export primary zone data from PowerDNS
2. Customer imports into their primary DNS provider
3. Update PowerDNS zone to type=Slave, masters=[customer_primary_ip]
4. Wait for first AXFR
5. Customer adds platform NS as secondary at registrar
...
3. Admin creates secondary zone pointing to customer's NS
4. Customer adds platform NS as secondary
5. Monitor AXFR

### Secondary → Primary

1. Export zone from PowerDNS secondary
2. Create new Primary zone in PowerDNS from export
3. Customer updates registrar to use platform NS
...
- **Check:** `dig acme.com NS` — should list platform NS
...
- **Fix:** Platform should auto-create MX; check PowerDNS zone
...
- **Check:** PowerDNS logs: `journalctl -u powerdns | grep AXFR`
- **Fix:** Verify primary NS IP, check firewall, enable AXFR in primary

**Symptom:** Zone stuck (stale data)
- **Check:** `pdnsutil list-zone acme.com` — check serial
- **Fix:** Manually trigger AXFR: `pdnsutil increase-serial acme.com`
...
- [ ] **Conditional Logic:**
  - [ ] Primary: Create PowerDNS zone (type=Primary)
  - [ ] CNAME: Skip PowerDNS zone, set cname_target
  - [ ] Secondary: Create PowerDNS zone (type=Slave), set masters IP
- [ ] **SSL Certificate Handling:**
  - [ ] Primary: DNS-01 ACME via PowerDNS
  - [ ] CNAME: HTTP-01 ACME
...
- [ ] **Customer Panel:**
  ...
  - [ ] Display nameserver info (if primary mode)
  - [ ] Display CNAME target (if CNAME mode)
  - [ ] Display secondary NS info (if secondary mode)
