...
| `{{platform.ns1}}` | Primary nameserver FQDN | `ns1.platform.com` |
| `{{platform.ns2}}` | Secondary nameserver FQDN | `ns2.platform.com` |
...
records:
  ...
  #
  # PowerDNS requirement: lua-records=yes must be set in pdns.conf (enabled by default
  # in this platform — see POWERDNS_INTEGRATION.md §LUA Records Configuration).
  # LUA records have no negative performance impact at this scale and no security
  # implications given the PowerDNS API is mesh-only (ADR-013).
...
```
1. Zone created in PowerDNS (SOA + NS records only)
      ↓
...
4. Template records rendered (all {{variables}} substituted)
      ↓
5. Records POSTed to PowerDNS API in a single batch request
      ↓
6. Zone propagated to external slaves via AXFR
      ↓
...
## Template Application: PowerDNS API Batch Request

All template records are applied in a single `PATCH` to the PowerDNS zone endpoint:
...
Both interfaces provide full CRUD on all record types: A, AAAA, CNAME, MX, TXT, SRV, CAA, NS, and any other standard type supported by PowerDNS. There is no concept of "template-locked" or "platform-managed" records — everything is editable.
...
- Any failures are reported per-domain with the reason (e.g. PowerDNS API error); the operation continues for remaining domains
...
- [ ] Implement batch PowerDNS PATCH request for template records (non-destructive: REPLACE per rrset, never DELETE zone records)
...
- [ ] Validate record content server-side before writing to PowerDNS (type-specific format checks)
- [ ] Propagation status indicator per record (show when change has reached all nameservers)
