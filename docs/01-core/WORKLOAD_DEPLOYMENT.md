...
Multiple Starter clients share a pool of Apache+PHP pods. Apache VirtualHost config routes requests to each client's document root on their persistent volume.

**How it works:**
- Multiple clients in a single Apache+PHP container
- Each client's PV mounted as subdirectory under `/mnt/clients/`
- VirtualHost configs auto-generated by Management API
- Apache graceful restart on config changes
...
| Shared pod replicas | **2 pods per pool (maximum)** — higher replica counts multiply the number of simultaneous Longhorn PV mounts (replicas × clients), which increases storage I/O overhead without proportional traffic benefit. Use 2 replicas for HA; add a new pool instead of increasing replicas per pool. |
...
Clients on Starter plans are limited to Apache+PHP containers. Business/Premium clients can select from any dedicated pod catalog entry.
...
| --- | --- | --- |
| **Starter** | Shared pod | Apache+PHP variants only (clients select version) |
