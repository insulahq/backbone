...
| 2 | Install k3s single-node cluster | `infrastructure` `phase:1` | DevOps |
| 3 | Install Longhorn storage (block storage) | `infrastructure` `phase:1` | DevOps |
...
| 12 | Install Flux v2 (GitOps controller) | `infrastructure` `phase:1` | DevOps |
...
└── modules/
    ...
    └── dns/                 # Hetzner DNS (NS delegation records)
```
...
- Implement authentication (JWT + Dex OIDC)
...
- Jobs: lint → typecheck → migrations → unit tests → integration tests → Docker build → Trivy scan → push to Harbor
...
By end of Week 12:
- Complete infrastructure on Hetzner (k3s, Longhorn, NGINX, PowerDNS, Harbor, Flux, monitoring)
