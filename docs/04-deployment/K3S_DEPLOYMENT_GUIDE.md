...
1. **NetBird connectivity** — workstation must be on the mesh:
   ```bash
   netbird status
   # Must show "Connected" and all 3 peers visible
   ```
...
**What happens:**
1. k3s binary is downloaded and installed
2. k3s systemd service is enabled and started
3. Platform and ingress-nginx namespaces are created
...
3. **Scale workers** — add worker nodes if needed (currently not required for Phase 1)
