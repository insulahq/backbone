...
8. [Networking & DNS Management](#networking-dns)
9. [Security & Access Control](#security-access)
10. [Monitoring, Logging & Alerts](#monitoring-logging)
11. [CI/CD & Container Registry](#cicd-registry)
12. [Email & Communication](#email-communication)
13. [Backup & Disaster Recovery](#backup-recovery)
14. [Subscription & Expiry Management](#subscription-expiry-management)
15. [Audit & Compliance](#audit-compliance)
16. [Bulk Operations](#bulk-operations)
17. [Advanced Search & Filtering](#advanced-search)
18. [Branding & Customization](#branding--customization)
19. [Customizable Dashboards & Widgets](#customizable-dashboards--widgets)
20. [Authentication: Passwordless Login with OIDC](#authentication-passwordless-login-with-oidc)
21. [Mobile Optimization & Responsive Design](#mobile-optimization--responsive-design)
22. [Theme Customization - Light & Dark Mode](#theme-customization---light--dark-mode)
...
| **Platform pod overview** | Quick-view of all platform service pods (PowerDNS, Harbor, mail, Dex, etc.) with health badge | 1 |
...
Storage class: `longhorn`. Client workload PVCs are mounted at `/mnt/clients/client-{id}/` inside shared pods (Starter) or at the container's working directory in dedicated pods (Business/Premium). Plan storage defaults: Starter 5 Gi, Business 20 Gi, Premium 50 Gi.
...
| Mount path | e.g. `/mnt/clients/client-acme-corp/` (Starter) or workload path |
...
✓ Installing PowerDNS...                        (15s)
✓ Configuring NetBird peer...                   (10s)
...
| **Auto-bootstrap (master)** | Full stack: k3s + NGINX + Longhorn + cert-manager + Management API + PowerDNS + NetBird (14 steps) | 1 |
...
**Requirement:** Manage the platform's PowerDNS authoritative DNS infrastructure — master health, zone list, per-zone record editing, DNSSEC management, external slave sync status, zone template administration, and AXFR/RNDC controls.

DNS zone template management (view/edit the global template, re-apply to domains) is fully specified in **ND.3 DNS Zone Template Management**. ND.1 covers the DNS server layer: health, zones, records, DNSSEC, and slave sync.

See also: `POWERDNS_INTEGRATION.md`, `DNS_ZONE_TEMPLATES.md`, `SLI_SLO_DEFINITION.md` (DNS SLO: **99.95% availability**, p95 query latency **< 50 ms**).

**Features:**

| Feature | Specification | Phase |
|---------|---------------|-------|
| **PowerDNS health status** | Master pod status, zone count, API latency, AXFR notify queue, last config reload | 1 |
...
| **Alert surface** | `PowerDNSMasterDown`, `PowerDNSAPILatencyHigh`, `PowerDNSExternalSlaveDown`, `PowerDNSAXFRFailure`, `DNSSECDSRecordMissing` | 1 |

---

#### PowerDNS Health Status

**Admin Panel → DNS → Overview**

| Field | Example | Notes |
|-------|---------|-------|
| Master pod | `● powerdns-master-0 running` | Critical banner if pod not running |
...
| Last config reload | `2026-03-08 01:00 UTC` | |

Prometheus alerts surfaced here: `PowerDNSMasterDown` (Critical), `PowerDNSAPILatencyHigh` (Warning), `PowerDNSExternalSlaveDown` (Critical), `PowerDNSAXFRFailure` (Critical), `DNSSECDSRecordMissing` (Warning), `DNSSECValidationFailed` (Critical).
...
Button on the zone detail page. Calls `rndc notify <zone>.` on the PowerDNS master. The notify queue drains within seconds under normal conditions. Propagation monitor (SOA polling) activates and shows per-nameserver confirmation status:
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/v1/admin/dns/health` | PowerDNS master status, zone count, API latency, SLO availability (30d), AXFR queue |
...
**Requirement:** Operational management of the Dex OIDC provider — the platform identity layer that underpins all admin, staff, and client authentication. This section covers the infrastructure view: Dex health, signing keys, registered OIDC clients, and connector status.
...
See also: `03-security/SECURITY_ARCHITECTURE.md` (JWT validation, JWKS config, token lifecycle) and `ARCHITECTURE_DECISION_RECORDS.md` ADR-004 (Dex deployment decision).

**Features:**

| Feature | Specification | Phase |
|---------|---------------|-------|
| **Dex health status** | Real-time pod and endpoint health for the Dex deployment | 1 |
| **JWKS / signing key status** | Current signing key ID, algorithm, age, and next rotation | 1 |
| **Registered OIDC clients** | List of applications registered as OIDC clients in Dex | 1 |
...
| **Force signing key rotation** | Immediately rotate Dex signing keys (emergency action) | 2 |
...
| Field | Value |
|-------|-------|
| Dex pods | `3/3 Running` — pod count and status |
| Issuer URL | `https://dex.platform.com` — clickable to `/.well-known/openid-configuration` |
| JWKS endpoint | `● Reachable` / `✗ Unreachable` — live HTTP check |
| Database (PostgreSQL) | `● Connected` / `✗ Error` — `postgres-auth-0` in namespace `auth` |
| Namespace | `auth` |
| Last restart | Timestamp of most recent pod restart |

An inline alert banner appears if any component is unhealthy, with a link to the `Authentication/OIDC Failures` section of the incident response runbook.

---

#### JWKS / Signing Key Status

Dex signs all JWTs with an RS256 RSA key stored in the `dex-keys` Secret in the `auth` namespace. The admin panel reads the current key metadata (not the private key material) via the Dex API.
...
| Rotation policy | Configurable in Dex — default: every 6 hours (Dex rotates automatically) |
| Previous key (`kid`) | Prior key ID still accepted during rotation overlap window |

> **Emergency key rotation:** If a signing key is suspected compromised, the admin can trigger immediate rotation via the "Force Key Rotation" action. This deletes the `dex-keys` Secret — Dex regenerates new keys on startup. All existing access tokens become invalid immediately; refresh tokens remain valid and will obtain new access tokens signed with the new key on next use.

---

#### Registered OIDC Clients

Every application that authenticates via Dex must be registered as a static client with a client ID, secret, and allowed redirect URIs. The admin panel shows the current registered client list (read-only — changes require a Dex config redeploy).
...
> **Adding or modifying a client** requires updating the Dex `config.yaml` and redeploying. The panel surfaces what is configured — it does not provide a live editor for Dex static clients. This is intentional: static client changes are infrastructure changes that go through GitOps (Flux), not through the admin UI.

---

#### Connector Status

Each upstream identity provider configured in Dex is a "connector". The panel shows the health of each by performing a test discovery/token exchange.
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/v1/admin/oidc/health` | Dex pod status, issuer URL reachability, database connection, JWKS endpoint |
| `GET` | `/api/v1/admin/oidc/keys` | Current and previous signing key metadata (no private key material) |
| `POST` | `/api/v1/admin/oidc/keys/rotate` | Force immediate signing key rotation (deletes `dex-keys` Secret, triggers Dex restart) |
| `GET` | `/api/v1/admin/oidc/clients` | List registered Dex static clients |
...
**Requirement:** Surface WireGuard mesh health, peer status, and topology within the admin panel. Full peer management (access policies, ACLs, OIDC config, setup key issuance) is handled in the native NetBird dashboard — the admin panel links to it and exposes the health/status subset that is most relevant during day-to-day operations and incident response. See `03-security/SECURITY_ARCHITECTURE.md` and `ARCHITECTURE_DECISION_RECORDS.md` ADR-013 for the full NetBird architecture.

**Data source:** The admin panel backend calls the **NetBird Management API** (self-hosted on VPS 1, reachable via the mesh at `http://netbird-mgmt.platform.internal`) to retrieve peer and server state. Data is refreshed on page load and auto-polled every 60 seconds while the page is open.
...
All endpoints proxy to the NetBird Management API via the platform backend — the frontend never calls NetBird directly.
...
Grafana is also accessible directly at `https://grafana.platform.com` (OIDC SSO via Dex, client ID `grafana`). The admin panel provides an integrated subset — the most operationally relevant views — without requiring admins to navigate to Grafana for routine checks.
...
| Column | Notes |
|--------|-------|
| Alert name | e.g. `EmailIPBlacklisted`, `CertExpiryCritical`, `PowerDNSMasterDown` |
...
| `PowerDNSMasterDown` | Critical | DNS |
...
| **Dex (Internal)** | ✅ Built-in | OpenID Connect provider |
