...
| **Admin VPN (NetBird)** | Mesh peer count, Signal server reachability (VPS 1 + VPS 2 + home), TURN relay status, management API health |
| **DNS (PowerDNS)** | Query rate per server (ns1/ns2), AXFR sync status, zone serial mismatch, query latency, SERVFAIL rate |
...
| Critical alerts | Node down, cluster unhealthy, shared DB down, backup failure, cert expiry < 7d, disk > 90%, NetBird primary down, both DNS servers down, DNS AXFR sync failed > 1h |
