...
| **CNI Plugin** | **Flannel** (k3s default, simple overlay) → Calico upgrade path (for advanced network policies at scale) |
...
| Component | Decision |
| --- | --- |
| **CNI plugin** | **Flannel** (k3s default, simple) → **Calico upgrade** for advanced network policies at scale |
| Service mesh | Not initially — evaluate Linkerd if mTLS needed at scale |
| **Ingress controller** | **NGINX Ingress Controller** (with ModSecurity WAF) |
| **DNS (external)** | **PowerDNS with API** or **Cloudflare/Route53 integration** |
